...
#### ntfy 推送通知
- 会话回答完成、需要审批/输入、异常终止时自动推送通知到手机
- 设置页面支持配置 ntfy 服务器地址、Topic、启用开关
- 内置测试按钮，一键验证通知是否正常

#### 文件管理器增强
- **复制/剪切/粘贴** — 右键菜单支持文件和目录的复制、剪切、粘贴操作
- **拖拽移动** — 文件树内拖拽文件到目标目录直接移动
- **上传/下载** — 工具栏上传按钮 + 右键「上传到此目录」+ 右键下载文件
- **多选操作** — Ctrl+点击切换选中，Shift+点击范围选择，支持批量删除/复制/剪切/下载
- **Toast 通知** — 所有操作（复制、粘贴、删除、重命名、新建）成功/失败时弹出提示
- **行高设置** — 设置 → 外观 → 文件管理行高（24-48px 滑块），调整文件树行距
- **在此目录新建会话** — 右键目录/文件弹出选项，点击后注册项目并跳转新建会话
- **图片预览** — 选中图片文件时在编辑区直接显示预览（支持 png/jpg/gif/webp/svg）

### 修复
- **Android 文件管理器长按卡死** — 用透明遮罩拦截浏览器原生长按检测，通过 Pointer Events 手动实现长按/滚动/点击
- **用户回合导航混入系统消息** — `/compact` 命令和 `<system-reminder>` 等标签出现在用户回合列表中
- **「从此回合加载」不生效** — `maxMessages` 截断了目标消息，改为从末尾取
- **手机端语音输入图标快速闪烁** — 对象引用每次渲染变化导致 provider 重建，改用字符串 key 比较
- **Android 切回浏览器丢失最新消息** — WebSocket 重连时初始加载未完成，加了守卫检查
- **页面刷新后卡在「从此回合」位置** — `maxMessages` 从起点截断丢失尾部新消息，改为从末尾取
